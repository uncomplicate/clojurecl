  ...
  (facts
   ...
   (let [host-a (float-array [10])
     ...
     (with-release [cl-a (cl-buffer ctx (* 2 Float/BYTES) :read-only)
       ...
       (enqueue-write cqueue cl-a host-a) => cqueue
       (enqueue-write cqueue cl-b host-b) => cqueue
       (enqueue-nd-range cqueue double-test work-sizes) => cqueue
       (enqueue-read cqueue cl-out host-out) => cqueue
  ...
  (facts
   ...
   (let [host-data (byte-array 16)
     ...
     (with-release [cl-data (cl-buffer ctx 16 :write-only)
       ...
       (enqueue-write cqueue cl-data host-data) => cqueue
       (enqueue-nd-range cqueue vector-bytes work-sizes) => cqueue
       (enqueue-read cqueue cl-data host-data) => cqueue
